...
def get_candidates(pt, rn, search_dist):
    ...
    mbr = MBR(pt.lat - search_dist * LAT_PER_METER,
              ...
              pt.lng + search_dist * LNG_PER_METER)
    candidate_edges = rn.range_query(mbr)  # list of edges (two nodes/points)  # 这行代码用于筛选出位于特定区域内的候选边
    if len(candidate_edges) > 0:
        candi_pt_list = [cal_candidate_point(pt, rn, candidate_edge) for candidate_edge in candidate_edges] # 通过计算原始 GPS 点在道路段上的最近投影点，提供关键属性（距离、偏移量、比例）
        # refinement
        candi_pt_list = [candi_pt for candi_pt in candi_pt_list if candi_pt.error <= search_dist] # 只保留误差（error）小于等于 search_dist 的候选点
