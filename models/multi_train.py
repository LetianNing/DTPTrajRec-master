...
def train(model, spatial_A_trans, SE, iterator, optimizer, scheduler , log_vars, rn_dict, grid_rn_dict, rn,
    ...
    for i, batch in enumerate(iterator):
        ...
        curr_time = time.time()
        optimizer.zero_grad()
        src_gps_seqs = src_gps_seqs.to(device)
        output_ids, output_rates = model(src_gps_seqs,use_id_seq, spatial_A_trans, src_road_index_seqs, SE,
                                         tra_time_A, tra_loca_A, src_lengths, src_grid_seqs, src_eid_seqs,
                                         src_rate_seqs, trg_in_grid_seqs,
                                         trg_in_index_seqs, trg_rids, trg_rates, trg_lengths, pre_grids, next_grids,
                                         constraint_mat,
        ...
        curr_time = time.time()
        ttl_loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(),1)  # log_vars are not necessary to clip
        optimizer.step()
...
def evaluate(model, spatial_A_trans, SE, iterator, rn_dict, grid_rn_dict, rn, raw2new_rid_dict,
