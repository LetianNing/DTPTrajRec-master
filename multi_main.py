...
from models.model import DTPTrajRec, DecoderMulti, Encoder1, GaussianEstimator1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--z_dim', type=int, default=512, help='sample size')
    ...
    model_save_path = './results/' + 'chengdu' + args.module_type + '_kr_' + str(args.keep_ratio) + '_debug_' + str(
        args.debug) + \
    ...
    sample1 = GaussianEstimator1(args)

    dec = DecoderMulti(args, SE, torch.tensor(spatial_A_trans, dtype=torch.float))
    model = DTPTrajRec(enc1, sample1, dec, args, SE, args.hid_dim, args.max_xid, args.max_yid).to(device)
    ...
    if args.train_flag:
        ...
        for epoch in tqdm(range(args.n_epochs)):
            ...
            new_log_vars, train_loss, train_id_acc1, train_id_recall, train_id_precision, \
                train_rate_loss, train_id_loss = train(model, spatial_A_trans, SE, train_iterator,
                                                       optimizer, scheduler, log_vars,
            ...
            valid_id_acc1, valid_id_recall, valid_id_precision, valid_dis_mae_loss, valid_dis_rmse_loss, \
                ...
                valid_rate_loss, valid_id_loss = evaluate(model, spatial_A_trans, SE, valid_iterator,
                                                          rn_dict, grid_rn_dict, rn, raw2new_rid_dict,
    ...
    if args.test_flag:
        ...
        test_id_acc1, test_id_recall, test_id_precision, test_dis_mae_loss, test_dis_rmse_loss, \
            test_dis_rn_mae_loss, test_dis_rn_rmse_loss, test_rate_loss, test_id_loss = evaluate(model, spatial_A_trans, SE,
                                                                                                 test_iterator,
                                                                                                 rn_dict, grid_rn_dict,
                                                                                                 rn,
                                                                                                 raw2new_rid_dict,
