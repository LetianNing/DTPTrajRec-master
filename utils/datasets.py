...
class Dataset(torch.utils.data.Dataset):
    ...
    def get_data(self, trajs_dir, user_idx, norm_grid_poi_dict, norm_grid_rnfea_dict,
        ...
        if debug:
            print(1)
            trg_paths = os.listdir(trajs_dir)[:50]
            num = -1
        else:
            print(2)
            trg_paths = os.listdir(trajs_dir)
    ...
    def parse_traj(self, traj, user_id, norm_grid_poi_dict, norm_grid_rnfea_dict, weather_dict, win_size, ds_type, keep_ratio):
        ...
        for tr in new_trajs:
            ...
            # get source sequence
            # ds_pt_list 下采样之后得到的 15s 0.125 .... = 2min 0.0625 ..==4min 0.25..= 1min
        ...
        return (new_tid_ls, mm_gps_seq_ls, mm_eids_ls, mm_rates_ls, mm_t_ls, mm_index_ls, mm_grid_ls, mm_gps_ls,
                ls_grid_seq_ls, ls_gps_seq_ls, ls_road_index_seq_ls, ls_eid_seq_ls, ls_rate_seq_ls, features_ls)
    ...
    def get_trg_grid_t(self, all_pt_list, downsample_pt_list):
        ...
        for trg_pt in all_pt_list:
            ...
            result = filter(lambda x: x.time == trg_pt.time, downsample_pt_list)
